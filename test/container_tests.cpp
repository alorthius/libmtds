...
int producer_consumer_test(C& container, size_t num_threads, size_t num_operations) {
    size_t operations_per_thread = 2 * num_operations / num_threads;
    std::vector<std::thread> threads;
    std::vector<int> sums(num_threads);

    for (size_t i = 0; i < num_threads; ++i) {
        if (i % 2 == 0) {
            threads.emplace_back(multiple_push<C>, std::ref(container), operations_per_thread);
        } else {
            threads.emplace_back(multiple_pop, std::ref(container), operations_per_thread,
                                 std::ref(sums[i]));
        }
    }
    for (auto& thread: threads) {
